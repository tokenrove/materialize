    ...
    G::Timestamp: Lattice,
{
    ...
    let arranged_output = input.scope().region_named("ReduceHierarchical", |inner| {
        ...
        for b in buckets.into_iter() {
            ...
            stage = if validating {
                let (stage, errs) = build_bucketed_stage::<_, Result<Vec<Row>, (Row, u64)>>(
                    debug_name,
                    stage,
                    aggr_funcs.clone(),
                    b,
                ...
                build_bucketed_stage::<_, Vec<Row>>(debug_name, stage, aggr_funcs.clone(), b)
            }
        }
    ...
    R: MaybeValidatingRow<Vec<Row>, (Row, u64)>,
{
    ...
    negated_output
        .negate()
        .concat(&input.map(move |((k, h), r)| ((k, h), R::ok(r))))
